...
def export_neo4j():
    ...
    try:
        ...
        for template in templates:
            ...
            # action_apps is a JSON column, so it is likely already a list
            actions = template.action_apps if isinstance(template.action_apps, list) else [a.strip() for a in str(template.action_apps).split(",") if a.strip()]
