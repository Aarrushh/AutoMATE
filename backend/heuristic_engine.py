...
class HeuristicEngine:
    ...
    def calculate_complexity(trigger_app: str, action_apps: List[str]) -> int:
        ...
        penalty = 0
        if any(app.upper() in {c.upper() for c in complex_apps} for app in all_apps):
            penalty = 1
    ...
    def estimate_opex(trigger_app: str, action_apps: List[str]) -> float:
        ...
        premium_apps_upper = {p.upper() for p in premium_apps}
        all_apps = [trigger_app] + action_apps

        opex = 0.0
        for app in all_apps:
            if app.upper() in premium_apps_upper:
                opex += 15.0
